-- 143 — Separar del cuadre del técnico lo que se vendió DESPUÉS de su recibo.
-- Fecha: 2026-09-04 · pedido de David
--
-- EL PROBLEMA (reportado por David): el técnico llega a cuadrar y le sale que
-- debe plata, aunque recogió el valor completo del servicio. Lo que pasó es que
-- después de la recogida se le vendieron adicionales al cliente, y el cuadre se
-- los estaba cobrando A ÉL.
--
-- LA CAUSA: `generar_cuadre_tecnico` calcula `valor_a_cobrar` leyendo el
-- `servicios.valor_total` de HOY, no el que existía cuando el técnico estuvo
-- allí. La diferencia del cuadre compara ese valor contra lo que él recogió, así
-- que todo lo vendido después le aparece como faltante.
--
-- MEDIDO EN PRODUCCIÓN el 2026-09-04: de 897 filas de cuadre con recibo, **105
-- crecieron** después del recibo, por un total de **$6.362.960**. En casi todas,
-- el crecimiento coincide EXACTAMENTE con el valor de los `servicio_recordatorios`
-- creados después del recibo (MAX 26.000, PETER 61.000, LULU 61.000, Lupita
-- 61.000, MERLIN 35.000…). El caso más claro es LUCKY: recibo de 1.490.000,
-- recogió 1.490.000, y el cuadre le pedía 1.600.000.
--
-- 🔑 POR QUÉ ES SEGURO QUITARLO DEL CUADRE: el seguimiento del cobro pendiente
-- NO vive en el cuadre, vive en la cartera de Finanzas (`estado_pago` +
-- `valor_total − valor_pagado`), y ahí ya está bien. Verificado en los mismos
-- servicios: MAX saldo 26.000 PARCIAL, Lupita 61.000 PARCIAL, ROCKET 60.000
-- PARCIAL, FILIMONA 80.000 PARCIAL. Sacar esa plata de la cuenta del técnico no
-- la pierde de vista: sigue perseguida donde se persigue el cobro.
--
-- LA FORMA, elegida para no romper nada:
--   · NO se cambia el significado de `valor_a_cobrar` — sigue siendo lo que vale
--     el servicio. Cambiarlo habría movido el suelo a la cartera, al PDF y a la
--     pestaña Comisiones.
--   · Se AÑADE `valor_posterior_recibo`, con DEFAULT 0. Las filas que ya existen
--     —incluidas las de los cuadres CERRADOS, que son inmutables por diseño
--     (migr. 015)— se quedan en 0 y por tanto siguen pintándose y sumando
--     EXACTAMENTE igual que hoy. Un cuadre firmado no puede cambiar de cifras
--     por un despliegue.
--   · Solo la rama del RECIBO lo calcula. Las filas sin recibo y las canceladas
--     no tienen contra qué comparar y se quedan en el DEFAULT.
--   · Solo resta cuando el servicio SUBIÓ de precio. Que baje ya lo trata el
--     camino de `excesoValorARecoger`, que no se toca.

BEGIN;

ALTER TABLE public.cuadre_items
  ADD COLUMN IF NOT EXISTS valor_posterior_recibo numeric NOT NULL DEFAULT 0;

COMMENT ON COLUMN public.cuadre_items.valor_posterior_recibo
  IS 'Cuánto creció el valor del servicio DESPUÉS del recibo del técnico (adicionales vendidos cuando él ya no estaba). No es plata suya: se descuenta de lo que se le exige en el cuadre, pero sigue pendiente de cobro en la cartera. 0 en las filas anteriores a la migración 143 y en las que no tienen recibo.';
