-- ============================================================================
-- 019 — Cuadre: "valor a recoger" (neto) para la diferencia
-- Fecha: 2026-06-25
--
-- BUG (David): la columna Diferencia usaba `valor_a_cobrar` (BRUTO = valor_total
-- + comisión cuando la comisión fue descuento inmediato, migración 017). Eso hace
-- que la comisión aparezca como "faltante" aunque el técnico nunca debía recogerla.
--
-- Corrección: la diferencia debe calcularse contra el VALOR A RECOGER = lo que el
-- cliente realmente paga = `servicios.valor_total` (neto). En Registro la comisión
-- solo se resta de valor_total cuando es descuento inmediato; cuando no, valor_total
-- ya es el bruto. En ambos casos valor_total = lo que el técnico recoge del cliente.
--   diferencia = valor_a_recoger − total_cobrado
--
-- `valor_a_cobrar` (bruto) se conserva tal cual para mostrar el precio original
-- (igual que el recibo del técnico). HAY QUE REGENERAR el cuadre para poblar la
-- columna nueva en items existentes (los viejos quedan NULL → el front cae a
-- valor_a_cobrar como antes).
--
...
ALTER TABLE public.cuadre_items
  ADD COLUMN IF NOT EXISTS valor_a_recoger numeric;

COMMENT ON COLUMN public.cuadre_items.valor_a_recoger
  IS 'Neto que el cliente paga / el técnico recoge (= servicios.valor_total). Base de la diferencia. valor_a_cobrar es el bruto (con comisión) solo para mostrar.';

COMMIT;

-- ============================================================================
-- generar_cuadre_tecnico — + valor_a_recoger (= svc_valor_total). Resto idéntico
-- a la 018 (snapshot medios_pago + preservación de revisión/conciliación).
-- ============================================================================
