-- ============================================================================
-- 014 — Pago al técnico por servicio CANCELADO · Fase 1.4
-- Fecha: 2026-06-23
--
-- David: al técnico también se le paga por un servicio cancelado (viaje perdido).
--   - Monto FIJO único (no por plan ni vehículo): tarifas_reconocimiento_tecnico.pago_cancelado.
--   - Solo si el técnico YA HABÍA SALIDO: la cancelación ocurrió en etapa
--     EN_RECOGIDA o posterior (etapa_cancelacion <> 'INGRESADO'). Cancelados
--     antes de despachar al técnico NO pagan.
--   - Se incluye en el cuadro por FECHA DE CANCELACIÓN (cancelado_en) y por el
--     técnico de la recogida (recogidas.tecnico_id, cae a servicios.tecnico_id).
--   - Entra como reconocido al técnico: total_reconocido += total_cancelados.
--
-- Los cancelados NO tienen recibo ni dinero recibido: la fila va con cobrado=0 y
-- solo el pago por cancelación. (Si un cancelado tuviera dinero cobrado, ese caso
-- raro no se cuadra aquí — el dinero de cancelados suele devolverse.)
--
...
ALTER TABLE public.tarifas_reconocimiento_tecnico
  ADD COLUMN IF NOT EXISTS pago_cancelado numeric NOT NULL DEFAULT 0 CHECK (pago_cancelado >= 0);

ALTER TABLE public.cuadre_items
  ADD COLUMN IF NOT EXISTS es_cancelado boolean NOT NULL DEFAULT false;

ALTER TABLE public.cuadres_tecnico
  ADD COLUMN IF NOT EXISTS total_cancelados numeric NOT NULL DEFAULT 0;

COMMIT;

-- ============================================================================
-- generar_cuadre_tecnico — + servicios cancelados (viaje perdido)
-- ============================================================================
...
BEGIN
  ...
  -- ── Recibos del técnico (servicios prestados) ──────────────────────────────
  FOR v_rec IN
...
-- ============================================================================
-- set_cuadre_item_lejania — reconocido incluye pago_servicio + cancelados
-- ============================================================================
...
BEGIN
  ...
  IF v_es_cancelado THEN
    RAISE EXCEPTION 'ITEM_CANCELADO: una fila de servicio cancelado no admite recargo de lejanía';
  END IF;
  ...
  v_reconocido := v_tot_transporte + v_tot_recargos + COALESCE(v_tot_pago, 0) + COALESCE(v_tot_cancelados, 0);
