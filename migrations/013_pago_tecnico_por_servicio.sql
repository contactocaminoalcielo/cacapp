-- ============================================================================
-- 013 — Pago al técnico por servicio (plan × vehículo) · Fase 1.3
-- Fecha: 2026-06-23
--
-- David: a cada técnico se le paga un valor POR SERVICIO que depende del PLAN y
-- del VEHÍCULO. El vehículo NO es del servicio: viene del perfil del técnico
-- (personal.tipo_vehiculo), asignado al crear el rol. Como el cuadre es por
-- técnico, el vehículo es constante para todo el cuadre.
--
-- Modelo: tarifa por plan con dos montos (moto / carro). Mapa de vehículo:
--   personal.tipo_vehiculo = 'MOTO'           → monto_moto
--   cualquier otro (CAMIONETA/CARRO/NULL)     → monto_carro  (4 ruedas)
--
-- Entra a la matemática como un reconocido más al técnico:
--   total_reconocido = transporte + recargos + pago_servicio
--
...
-- 1. Tarifa de pago al técnico por servicio (un registro por plan).
CREATE TABLE IF NOT EXISTS public.tarifas_pago_tecnico_servicio (
  id          uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  plan_id     uuid NOT NULL UNIQUE REFERENCES public.planes(id) ON DELETE CASCADE,
  monto_moto  numeric NOT NULL DEFAULT 0 CHECK (monto_moto  >= 0),
  monto_carro numeric NOT NULL DEFAULT 0 CHECK (monto_carro >= 0),
  updated_at  timestamptz NOT NULL DEFAULT now()
);

DROP TRIGGER IF EXISTS trg_tarifas_pago_tecnico_updated ON public.tarifas_pago_tecnico_servicio;
CREATE TRIGGER trg_tarifas_pago_tecnico_updated
  BEFORE UPDATE ON public.tarifas_pago_tecnico_servicio
  FOR EACH ROW EXECUTE FUNCTION set_updated_at();

-- 2. Snapshot en el detalle + total en la cabecera.
ALTER TABLE public.cuadre_items
  ADD COLUMN IF NOT EXISTS pago_servicio numeric NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS vehiculo      text;

ALTER TABLE public.cuadres_tecnico
  ADD COLUMN IF NOT EXISTS total_pago_servicio numeric NOT NULL DEFAULT 0;

-- 3. GRANTs + RLS (patrón del proyecto).
GRANT ALL ON TABLE public.tarifas_pago_tecnico_servicio TO postgres, anon, authenticated, service_role;
ALTER TABLE public.tarifas_pago_tecnico_servicio ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "auth_full" ON public.tarifas_pago_tecnico_servicio;
CREATE POLICY "auth_full" ON public.tarifas_pago_tecnico_servicio FOR ALL TO authenticated USING (true) WITH CHECK (true);

COMMIT;

-- ============================================================================
-- generar_cuadre_tecnico — + pago al técnico por servicio (plan × vehículo)
-- ============================================================================
...
BEGIN
  ...
  -- Vehículo del técnico (constante para todo el cuadre).
  ...
  LOOP
    ...
    -- Pago al técnico por el servicio: tarifa del plan según vehículo del técnico.
    v_pago := 0;
    ...
    INSERT INTO public.cuadre_items (
      ...
      es_dominical, es_festivo, es_nocturno, es_lejania, recargo_aplicado, pago_servicio
    ) VALUES (
      ...
      v_es_dom, v_es_fes, v_es_noc, v_es_lej, v_recargo, v_pago
    );
  ...
  v_tot_reconocido := v_tot_transporte + v_tot_recargos + v_tot_pago;
...
-- ============================================================================
-- set_cuadre_item_lejania — reconocido ahora incluye total_pago_servicio
-- ============================================================================
...
BEGIN
  ...
  SELECT total_transporte, total_pago_servicio, efectivo_recibido, ajustes_manuales
    INTO v_tot_transporte, v_tot_pago, v_efectivo, v_ajustes
  FROM public.cuadres_tecnico WHERE id = v_cuadre_id;

  v_reconocido := v_tot_transporte + v_tot_recargos + COALESCE(v_tot_pago, 0);
