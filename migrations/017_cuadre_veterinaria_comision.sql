-- ============================================================================
-- 017 — Cuadre: veterinaria, comisión y total a cobrar BRUTO
-- Fecha: 2026-06-23
--
-- David: el cuadre debe mostrar la veterinaria y la comisión, y el "total a
-- cobrar" debe ser el BRUTO (antes de descontar la comisión). Hoy valor_a_cobrar
-- usaba servicios.valor_total, que cuando la comisión fue DESCUENTO_INMEDIATO ya
-- viene NETO. Regla (igual que el recibo del técnico):
--   a_cobrar_bruto = valor_total + (comision_descontada ? comision_aliado : 0)
--
...
ALTER TABLE public.cuadre_items
  ADD COLUMN IF NOT EXISTS veterinaria text,
  ADD COLUMN IF NOT EXISTS comision    numeric NOT NULL DEFAULT 0;

COMMIT;

-- ============================================================================
-- generar_cuadre_tecnico — + veterinaria, comisión y a cobrar bruto
-- ============================================================================
...
BEGIN
  ...
  LOOP
    ...
    -- A cobrar BRUTO: si la comisión fue descontada, valor_total es neto → sumarla.
