-- ============================================================================
-- 021 — Cuadre: incluir servicios recogidos por el técnico SIN recibo
-- Fecha: 2026-06-26
--
-- David: al generar el cuadre de un técnico faltaban ~10 servicios (p.ej. Hella
-- de Mario Alexander y Muñeca de Melany, ambos ENTREGADO/INGRESADO con pago
-- PENDIENTE). Causa: el cuadre se arma desde `recibos_tecnico`, y esos servicios
-- el técnico los recogió pero NUNCA generó recibo (quedaron pendientes de pago).
-- Sin recibo = invisibles en el cuadre. El estado de producción NO importa.
--
-- Corrección (confirmada con David):
--   - El cuadre suma, además de los recibos, los servicios que el técnico recogió
--     en el rango y que NO tienen NINGÚN recibo: se insertan como "sin recibo"
--     (recogido $0, diferencia = lo que falta por cobrar = el neto).
--   - Rango por `servicios.fecha_ingreso` (decisión de David).
--   - Entran AUTOMÁTICAMENTE a Conciliaciones (vía LLAMAR_COBRAR por defecto, o
--     FACTURACION_MENSUAL si la veterinaria es de facturación mensual).
--   - Reconocimientos (transporte/recargos/pago al técnico) = 0 en estos items:
--     son filas de control de cobro pendiente, no afectan el dinero a entregar.
--     (Si más adelante se quiere reconocer el viaje, se hace explícito.)
--
...
ALTER TABLE public.cuadre_items
  ADD COLUMN IF NOT EXISTS sin_recibo boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN public.cuadre_items.sin_recibo
  IS 'true = el técnico recogió el servicio pero NO emitió recibo (no cobró). Recogido $0, falta el neto. Entra a Conciliaciones para gestionar el cobro.';

COMMIT;

-- ============================================================================
-- generar_cuadre_tecnico — + tercer loop: servicios recogidos sin recibo
-- (Sobre la 020: idéntica salvo el nuevo loop y la columna sin_recibo.)
-- ============================================================================
...
BEGIN
  ...
  -- El técnico hizo la recogida pero nunca generó recibo. Se incluyen como
  -- "sin recibo": recogido $0, neto pendiente por cobrar → entran a Conciliaciones.
  -- Rango por fecha_ingreso. Reconocimientos en 0 (no alteran el dinero a entregar).
  FOR v_sr IN
  ...
  LOOP
    ...
    v_p := v_prev_svc -> (v_sr.servicio_id::text);
    v_via := COALESCE(v_p->>'via',
