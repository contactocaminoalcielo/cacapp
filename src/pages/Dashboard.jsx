...
import { AlertTriangle, TrendingUp, PlusCircle, Activity, Snowflake, Package, Truck } from 'lucide-react'

const ESTADOS_ACTIVOS = ['INGRESADO','EN_RECOGIDA','EN_CUARTO_FRIO','EN_PROCESO','EN_PRODUCCION','LISTO','EN_ENTREGA']
...
export default function Dashboard() {
  ...
  async function cargar() {
    try {
      setLoading(true)
      const [{ data: kanban }, { data: alts }] = await Promise.all([
        db.from('v_kanban').select('*').order('fecha_ingreso', { ascending: false }),
        db.from('v_alertas').select('*').in('nivel_alerta', ['VENCIDO','HOY','URGENTE']).order('dias_para_vencer', { ascending: true }),
      ])
      setServicios(kanban || [])
      setAlertas(alts || [])
    } catch (e) {
  ...
  const activos       = servicios.filter(s => ESTADOS_ACTIVOS.includes(s.estado))
      ...
      <div className="flex-1 p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-4">
          ...
          <StatCard
            ...
            valueColor={alertas.length > 0 ? '#DC2626' : '#9CA3AF'}
            icon={AlertTriangle}
          />
          <StatCard
            label="Entregados"
          ...
          <StatCard
            label="Cuarto frío"
            value={enCuartoFrio.length}
            sub="En refrigeración"
            valueColor="#06B6D4"
            icon={Snowflake}
          />
        </div>
                    ...
                    >
                      ...
                      <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full border flex-shrink-0 ${
                        ...
                        {a.dias_para_vencer < 0
                          ? `${Math.abs(a.dias_para_vencer)}d vencido`
                          : a.dias_para_vencer === 0 ? 'Hoy'
                          : `${a.dias_para_vencer}d`}
                      </span>
                    </button>
                  ))}
                  {alertas.length > 7 && (
                    <button
                      className="w-full text-center text-[12px] text-gray-400 hover:text-gray-600 py-2 transition-colors"
                      onClick={() => navigate('/kanban')}
                    >
              ...
              {ESTADOS_ACTIVOS.map(estado => {
                ...
                return (
                  <div key={estado} className="flex items-center gap-3">
                    <span className="text-[12px] font-medium text-gray-600 w-28 flex-shrink-0 truncate">
                      {meta.label}
                    </span>
                    ...
                    <span className="text-[12px] font-bold text-gray-700 w-5 text-right tabular-nums">
                      {count}
                    </span>
                  </div>
                )
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            ...
            <button
              className="text-[12px] text-gray-400 hover:text-[#3D5A27] font-medium transition-colors"
              onClick={() => navigate('/kanban')}
            >
              Ver todo →
            </button>
          ...
          <TableWrap>
            <Table>
              ...
              <tbody>
                {recientes.map(s => (
                  <Tr key={s.servicio_id} onClick={() => navigate('/kanban')}>
                    <Td>
