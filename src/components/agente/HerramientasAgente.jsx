...
export default function HerramientasAgente({ agenteId }) {
  ...
  if (cargando) return <div className="min-h-40 grid place-items-center text-neutral-500"><Loader2 className="animate-spin" /></div>
    ...
    <section className="rounded-2xl border bg-white p-5 shadow-sm space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex gap-3">
          ...
          <div><h3 className="font-semibold text-neutral-900">Capacidades</h3><p className="text-sm text-neutral-600">Permisos que el modelo puede solicitar durante una conversación.</p></div>
        </div>
      ...
      <div className="space-y-3">
        {lista.map(h => (
          <article key={h.clave} className={`rounded-xl border p-4 ${h.activa ? 'border-[#C0D0F0] bg-[#F8FAFF]' : 'border-neutral-200'}`}>
            <div className="flex items-start gap-3">
              <input id={`tool-${h.clave}`} type="checkbox" checked={h.activa}
                onChange={e => cambiar(h.clave, 'activa', e.target.checked)}
                className="mt-1 h-5 w-5 cursor-pointer rounded border-neutral-300 accent-[#1A5CD8]" />
              <div className="min-w-0 flex-1">
                <label htmlFor={`tool-${h.clave}`} className="cursor-pointer text-sm font-semibold text-neutral-900">{h.nombre}</label>
                <p className="mt-0.5 text-xs leading-relaxed text-neutral-600">{h.resumen}</p>
                <code className="mt-1 inline-block text-[11px] text-neutral-500">{h.clave}</code>
                ...
                {h.activa && (
                  <div className="mt-3">
                    <label htmlFor={`tool-desc-${h.clave}`} className="block text-xs font-semibold text-neutral-700 mb-1">Descripción opcional para el modelo</label>
