...
const migration = readFileSync(new URL('../migrations/148_cierre_ecogrupal.sql', import.meta.url), 'utf8')
...
INSERT INTO planes VALUES (1,'ECO_GRUPAL'),(2,'OTRO');
...
DO $$ BEGIN
 IF (SELECT count(*) FROM servicios WHERE estado='ENTREGADO')<>2 THEN RAISE EXCEPTION 'Cierre incorrecto'; END IF;
 IF EXISTS (SELECT 1 FROM servicios WHERE estado='ENTREGADO' AND id NOT IN (lpad('1',32,'0')::uuid,lpad('2',32,'0')::uuid)) THEN RAISE EXCEPTION 'Cerró un servicio incompleto'; END IF;
...
UPDATE servicio_recordatorios SET estado='ENTREGADO' WHERE id IN (11,12);
DO $$ BEGIN
 IF (SELECT count(*) FROM servicios WHERE estado='ENTREGADO')<>4 THEN RAISE EXCEPTION 'No cerró al completar el último ítem'; END IF;
END $$;
...
END $$;
SELECT 'OK: ambos órdenes, envíos fallidos, pendientes, adicionales, NA, removidos, cancelados, otro plan y reenvíos' AS resultado;
ROLLBACK;
`)
